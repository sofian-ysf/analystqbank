...
import { ParsedAnkiCard, ParsedAnkiDeck } from './types'
...
/**
 * Parse notes from the database (including tags for topic mapping)
 */
function parseNotes(db: Database): AnkiNote[] {
  const result = db.exec('SELECT id, flds, mid, tags FROM notes')
...
function convertToCards(
  ...
  for (const card of cards) {
    ...
    // Parse tags from note
    const tags = note.tags.trim().split(/\s+/).filter(t => t.length > 0)
